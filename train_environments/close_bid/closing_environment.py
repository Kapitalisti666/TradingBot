...
class TradingBotRL(gym.Env):
    ...
    def reward_option_1(self):
        ...
        if position_action == 0:
            ...
            self.reward -= self.holding_bars * 0.001
            
            if self.holding_bars >= 10:
                self.reward -= 0.002 * np.exp(0.15 * (self.holding_bars - 10))
    ...
    def step(self, action):
        ...
        new_row = self.env_data.iloc[self.row_number][self.scale_attributes + ["hour_sin", "hour_cos", "trend_agreement", "ema_diff_2"]].copy().to_frame().T
    ...
    def episode_plot(self, reward_dict):
        ...
        logger.info(f"Environment {self.env_number} Total Profit: {total_profit}")
    ...
    def reset(self, seed=None, options=None):
        ...
        self.bid_placed_index = self.row_number
...
if __name__ == "__main__":
    ...
    dataset = pd.read_excel("envs/combined_dataset_full_with_meta_predictions_2025_05_31_10_55_34.xlsx", 
                            nrows=10000
                            )

    env = TradingBotRL(dataset = dataset)
